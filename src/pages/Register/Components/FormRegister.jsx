...
export default function FormRegister() {
    const [cadastroType, setCadastroType] = useState('USER');

    const renderForm = () => {
        switch (cadastroType) {
            case 'USER':
                return <FormUser />; // Renderiza o Formulário de Usuário
            case 'EMPRESA':
                return <FormWizard />; // Renderiza o Formulário de Empresa (Seu FormWizard)
            default:
                return <FormUser />; 
        }
    };
        ...
        <>
            <section className='w-full lg:w-[50%] h-full flex flex-col gap-10 pb-10 lg:pt-10 items-baseline min-h-screen'>
                ...
                <section className='shadow-custom-sm w-[90%] md:w-[85%] m-auto px-10 py-5 rounded-lg'>
                    ...
                    <div className='mb-7'>
                        <TypeRegister 
                            activeType={cadastroType} 
                            onSelect={setCadastroType} // A função que muda o estado
                        />
                    </div>
