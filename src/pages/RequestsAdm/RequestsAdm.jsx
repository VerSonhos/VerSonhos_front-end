...
    // Garantir que AGUARDANDO_APROVACAO seja tratado como PENDENTE para exibição
...
    
    // Lógica para definir cor e texto do botão de Confirmação baseado no statusNovo
...
    } else if (statusNovo === 'REPROVADO') {
        corConfirmar = 'bg-red-500 hover:bg-red-600';
        textoConfirmar = 'Recusar';
    } else if (statusNovo === 'CANCELADO') {
        corConfirmar = 'bg-red-700 hover:bg-red-800'; // Cor para Cancelamento
...
    const isSuccess = status === 'CONFIRMADO' || status === 'CONCLUIDO' || status === 'CANCELADO' || status === 'REPROVADO';
...
                    // MODIFICAÇÃO 1: Mapear o campo de observações. 
                    // Assumindo que o nome do campo é 'observacoes'.
                    // Se o nome for diferente, altere 'item.observacoes' para o nome correto.
...
                                        {/* MODIFICAÇÃO 2: Renderização condicional do campo Observações */}
                                        {itemSelecionado?.observacoes && (
                                            <div>
                                                <p className="font-semibold text-gray-700">Observações</p>
                                                {/* Usando whitespace-pre-wrap para preservar quebras de linha/espaços do usuário */}
...
                                            
                                            {/* LÓGICA DE TRANSIÇÃO DE STATUS: PENDENTE -> CONFIRMADO/REPROVADO */}
                                            {(itemSelecionado?.status === "PENDENTE" || itemSelecionado?.status === "AGUARDANDO_APROVACAO") && (
                                                <>
                                                    <button 
...
                                            {itemSelecionado?.status === "CONFIRMADO" && (
                                                <>
                                                    <button 
                                                        onClick={() => handleStatusChange("CONCLUIDO")} 
                                                        className="py-2 px-3 sm:px-4 rounded-lg text-white font-bold transition-colors duration-200 bg-green-700 hover:bg-green-800 text-xs sm:text-sm"
                                                    >
                                                        Concluir
                                                    </button>
                                                    <button 
                                                        onClick={() => handleStatusChange("CANCELADO")} 
                                                        className="py-2 px-3 sm:px-4 rounded-lg text-white font-bold transition-colors duration-200 bg-red-700 hover:bg-red-800 text-xs sm:text-sm"
                                                    >
                                                        Cancelar
                                                    </button>
                                                </>
                                            )}
                                            
                                            {/* STATUS EXCEPCIONAIS que podem ser CANCELADOS, mas não concluídos/aprovados */}
                                            {itemSelecionado?.status === "REPROVADO" && (
                                                <p className="text-gray-500 text-xs">O status atual não permite novas alterações.</p>
                                            )}

                                            {/* Para Concluído, Cancelado e Expirado: Nenhuma ação permitida */}
                                            {["CONCLUIDO", "CANCELADO", "EXPIRADO"].includes(itemSelecionado?.status) && (
                                                <p className="text-gray-500 text-xs">O status atual não permite novas alterações.</p>
                                            )}
                                        </div>
                                    </div>
