...
export default function Header() {
    ...
    if (isAuthenticated) {
        authLinkText = 'Painel';
        authLinkPath = role === 'admin' ? '/painelAdmin' : '/painelUsuario';
    }

    return (
        <header className={`transition-all ease-in-out z-50 px-8 bg-white-custom shadow-custom-md shadow-black/35 font-inter flex justify-between items-center h-[65px] fixed
        ${isScrolled
            ...
            <section className='w-[70px] sm:w-[200px] md:[225px]'>
                <Link className='w-full h-full' to={'/'}>
                    ...
                    <img className='sm:hidden w-full' src={IconeLogo} alt="Ícone logotipo VerSonhos" />
                </Link>
            </section>
            ...
            <section className='hidden md:flex justify-evenly w-[25%]'>
                ...
                <Link className='flex justify-center items-center gap-2 py-1 w-[50%] shadow-md text-white-custom bg-tertiary rounded-full hover:bg-tertiary-500 transition ease-in-out' to={authLinkPath}> 
                    <FaUser /> {authLinkText}
                </Link>
            </section>
            ...
            <section className='md:hidden flex items-center justify-end w-[70%] gap-2 sm:gap-0'>
                <div className='flex justify-evenly w-[80%]'>
                    <Link className='flex justify-center items-center gap-1 py-1 w-full sm:w-[40%] shadow-md text-eleventh border-3 border-eleventh rounded-full hover:bg-eleventh-500 hover:text-white-custom active:bg-eleventh-300 active:border-eleventh-100 active:text-white-custom transition ease-in-out' to={'/doar'}> 
                        <MdFavorite className='text-xl'/> Doar
                    </Link>

                    <Link className='hidden sm:flex justify-center items-center gap-2 py-1 w-[50%] shadow-md text-white-custom bg-tertiary rounded-full hover:bg-tertiary-500 transition ease-in-out' to={authLinkPath}> 
                        <FaUser /> {authLinkText}
                    </Link>
                </div>
            ...
            >
                <div className='w-full flex justify-between items-center px-5 py-1 mb-5 bg-tertiary'>
                    ...
                    <div className='w-[60%]'>
                        <Link 
                            className='flex sm:hidden justify-center items-center gap-2 py-1 w-full shadow-mdshadow-md text-white-custom border-3 border-white-custom rounded-full hover:bg-white-custom hover:text-tertiary
